...
class BedrockAuthenticationIntegrationTest {
    ...
    void verifiesChainAndActivatesBidirectionalCipher() throws Exception {
        ...
        String claims = "{\"nbf\":" + now.minusSeconds(5).getEpochSecond()
                + ",\"exp\":" + now.plusSeconds(60).getEpochSecond()
                + ",\"identityPublicKey\":\"" + clientKey
        ...
        var verifier = new BedrockChainVerifier(
        ...
        var serverPublic = KeyFactory.getInstance("EC").generatePublic(new X509EncodedKeySpec(
