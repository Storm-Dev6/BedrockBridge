...
public final class RakNetFrameCodec {
    ...
    public void encode(RakNetFrame frame, ByteBuffer output) {
        ...
        int flags = frame.reliability().ordinal() << 5;
        if (frame.split() != null) {
            flags |= SPLIT_FLAG;
        }
        output.put((byte) flags).order(ByteOrder.BIG_ENDIAN).putShort((short) (payload.remaining() * 8));
    ...
    public RakNetFrame decode(ByteBuffer input) {
        ...
        Reliability reliability = Reliability.values()[reliabilityId];
        ...
        if ((flags & SPLIT_FLAG) != 0) {
            require(input, 10);
            split = new RakNetFrame.SplitInfo(input.getInt(), Short.toUnsignedInt(input.getShort()), input.getInt());
        }
