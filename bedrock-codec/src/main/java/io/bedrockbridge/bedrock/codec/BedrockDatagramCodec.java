...
public final class BedrockDatagramCodec {
    ...
    public Packet decode(ByteBuffer datagram, PacketDirection direction) {
        ...
        Packet packet = processor.decode(
                BedrockProtocol.HANDSHAKE_VERSION,
    ...
    public int encode(Packet packet, ByteBuffer output) {
        ...
        if (usesOfflineMagic(packet.packetId())) {
            output.put(BedrockProtocol.OFFLINE_MESSAGE_MAGIC);
        }
    ...
    private static void requireMagic(ByteBuffer input) {
        if (input.remaining() < BedrockProtocol.OFFLINE_MESSAGE_MAGIC.length) {
            throw new BedrockValidationException("Truncated offline message magic");
        }
        byte[] actual = new byte[BedrockProtocol.OFFLINE_MESSAGE_MAGIC.length];
        input.get(actual);
        if (!Arrays.equals(actual, BedrockProtocol.OFFLINE_MESSAGE_MAGIC)) {
