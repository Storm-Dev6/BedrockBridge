...
/** Exact control-flow packet catalog for Mojang Bedrock r/26_u3 network protocol 1001. */
public final class BedrockProtocol1001PacketRegistry {
  private BedrockProtocol1001PacketRegistry() {}

  /** Builds the documented login and empty-resource-pack flow under the supplied limits. */
  ...
  private static <T extends BedrockPlayPacket> BedrockPlayPacketRegistration<T> registration(
      int packetId,
    ...
    return new BedrockPlayPacketRegistration<>(
        BedrockProtocol.PLAY_VERSION_1001, packetId, direction, states, type, codec);
  }
  ...
  private static BedrockPlayPacketCodec<DisconnectPacket> disconnectCodec(
    ...
    return new BedrockPlayPacketCodec<>() {
      ...
      public void encode(DisconnectPacket packet, BedrockBinaryWriter writer) {
        ...
        writer.writeUnsignedIntLE(packet.skipMessage() ? 1 : 0);
      ...
      public DisconnectPacket decode(BedrockBinaryReader reader) {
        ...
        long messageAlternative = reader.readUnsignedIntLE();
        if (messageAlternative == 1) {
          return DisconnectPacket.silent(reason);
        }
        if (messageAlternative != 0) {
          throw new BedrockValidationException("Unknown disconnect message alternative");
        }
  ...
  private static BedrockPlayPacketCodec<ResourcePacksInfoPacket> resourcePacksInfoCodec(
    ...
    return new BedrockPlayPacketCodec<>() {
      ...
      public void encode(ResourcePacksInfoPacket packet, BedrockBinaryWriter writer) {
        if (!packet.resourcePacks().isEmpty()) {
          throw new BedrockValidationException(
              "Protocol 1001 nonempty resource-pack catalogs require UUID packet data");
        }
        ...
        writer.writeLongLE(0);
        writer.writeString("", limits.maximumStringBytes());
        writer.writeUnsignedShortLE(0);
      }
      ...
      public ResourcePacksInfoPacket decode(BedrockBinaryReader reader) {
        ...
        boolean forceDisableVibrantVisuals = reader.readBoolean();
        long templateMostSignificantBits = reader.readLongLE();
        long templateLeastSignificantBits = reader.readLongLE();
        String templateVersion = reader.readString(limits.maximumStringBytes());
        int packCount = reader.readUnsignedShortLE();
        if (forceDisableVibrantVisuals
            || templateMostSignificantBits != 0
            || templateLeastSignificantBits != 0
            || !templateVersion.isEmpty()
            || packCount != 0) {
          throw new BedrockValidationException(
              "Protocol 1001 resource-pack catalog exceeds the supported empty flow");
        }
        return new ResourcePacksInfoPacket(required, addon, scripts, List.of());
      }
    };
  }
  ...
  private static BedrockPlayPacketCodec<ResourcePackStackPacket> resourcePackStackCodec(
    ...
    return new BedrockPlayPacketCodec<>() {
      ...
      public void encode(ResourcePackStackPacket packet, BedrockBinaryWriter writer) {
        if (!packet.addonPacks().isEmpty()) {
          throw new BedrockValidationException(
              "Protocol 1001 resource-pack stack has no separate addon-pack list");
        }
        ...
        for (BedrockExperiment experiment : packet.experiments()) {
          if (!experiment.alwaysOnName().isEmpty() || experiment.alwaysOnEnabled()) {
            throw new BedrockValidationException(
                "Protocol 1001 experiments contain one toggle per entry");
          }
        ...
        for (int index = 0; index < count; index++) {
          experiments.add(
              new BedrockExperiment(
                  reader.readString(limits.maximumStringBytes()), reader.readBoolean(), "", false));
        }
  ...
  private static void writeStackEntries(
    ...
    if (entries.size() > limits.maximumResourcePacks()) {
      throw new BedrockValidationException("Resource-pack stack exceeds configured limit");
    }
