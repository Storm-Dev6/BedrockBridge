...
public final class BedrockChainVerifier {
  ...
  public BedrockChainVerifier(
    ...
    if (this.trustedRoots.isEmpty()) {
      throw new IllegalArgumentException("At least one trust root is required");
    }
    ...
    if (clockSkew.isNegative()) {
      throw new IllegalArgumentException("clockSkew must be nonnegative");
    }
  }
  ...
  public AuthenticatedLogin verify(BedrockLoginPayload payload) {
    ...
    for (String compact : payload.chain()) {
      if (!tokens.add(compact)) {
        throw new BedrockValidationException("Duplicate JWT in login chain");
      }
  ...
  private static String requiredString(Map<String, Object> values, String key) {
    ...
    if (!(value instanceof String text) || text.isBlank()) {
      throw new BedrockValidationException("Missing string claim: " + key);
    }
    return text;
  }
  ...
  private static long requiredLong(Map<String, Object> values, String key) {
    ...
    if (!(value instanceof Long number)) {
      throw new BedrockValidationException("Missing integer claim: " + key);
    }
    return number;
  }
  ...
  private static Map<String, Object> requiredObject(Map<String, Object> values, String key) {
    ...
    if (!(value instanceof Map<?, ?>)) {
      throw new BedrockValidationException("Missing object claim: " + key);
    }
